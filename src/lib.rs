...
#[derive(Debug)]
pub struct Mesher<const CS: usize = 62> {
    // Output
    pub quads: [Vec<u64>; 6],
    // Internal buffers
    /// Self::CS_2 * 6
        ...
        for a in 1..(Self::CS_P-1) {
            ...
            for b in 1..(Self::CS_P-1) {
                ...
                for c in 1..(Self::CS_P-1) {
                    ...
                    self.face_masks[ba_index + 0 * Self::CS_2] |= face_value(v1, voxels[abc + Self::CS_P2], &transparents) << (c-1);
                    self.face_masks[ba_index + 1 * Self::CS_2] |= face_value(v1, voxels[abc - Self::CS_P2], &transparents) << (c-1);
                    
                    self.face_masks[ab_index + 2 * Self::CS_2] |= face_value(v1, voxels[abc + Self::CS_P], &transparents) << (c-1);
                    self.face_masks[ab_index + 3 * Self::CS_2] |= face_value(v1, voxels[abc - Self::CS_P], &transparents) << (c-1);
        
                    self.face_masks[ba_index + 4 * Self::CS_2] |= face_value(v1, voxels[abc + 1], &transparents) << c;
                    self.face_masks[ba_index + 5 * Self::CS_2] |= face_value(v1, voxels[abc - 1], &transparents) << c;
                }
            }
        }
    ...
    fn fast_face_culling(&mut self, voxels: &[u16], opaque_mask: &[u64], trans_mask: &[u64]) {
        ...
        for a in 1..(Self::CS_P-1) {
            ...
            for b in 1..(Self::CS_P-1) {
                ...
                self.face_masks[up_faces] = (opaque_col & !opaque_mask[ab + Self::CS_P]) >> 1;
                self.face_masks[down_faces] = (opaque_col & !opaque_mask[ab - Self::CS_P]) >> 1;
                ...
                while bits_here != 0 {
                    ...
                    let abc = ab_ + c;
                    let v1 = voxels[abc];
                    self.face_masks[up_faces] |= ((v1 != voxels[abc + Self::CS_P2]) as u64) << (c-1);
                    self.face_masks[down_faces] |= ((v1 != voxels[abc - Self::CS_P2]) as u64) << (c-1);
                    
                    self.face_masks[right_faces] |= ((v1 != voxels[abc + Self::CS_P]) as u64) << (c-1);
                    self.face_masks[left_faces] |= ((v1 != voxels[abc - Self::CS_P]) as u64) << (c-1);
...
mod tests {
    ...
    use crate::{self as bgm, debug_quad};
    ...
    fn doesnt_crash_sized() {
        let mut voxels = [0; bgm::Mesher::<CS_SIZED>::CS_P3];
        voxels[bgm::pad_linearize::<CS_SIZED>(0, 0, 0)] = 1;
        voxels[bgm::pad_linearize::<CS_SIZED>(0, 1, 0)] = 1;

        let mut mesher = bgm::Mesher::<CS_SIZED>::new();

        mesher.mesh(&voxels, &BTreeSet::default());
